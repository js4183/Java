...
public class SocketServer extends Thread {
	...
	public SocketServer(Socket socket) {// 소켓 넘버 등 정보를 넘기면서 서버소켓을 생성
		this.socket = socket;// 클라이언트가 접속시 소켓을 생성
	...
	public void run() {
		...
		try {
			...
			boolean check = false;// false-사용자가 최초로 입력하기전에 / true-닉네임을 입력한 후에
			...
			while ((readValue = reader.readLine()) != null) {
				if (check == false) {// 사용자가 입력한 내용을 nick 변수에 저장
					...
					for (Socket s : list) {
						...
						// (OutputStream s 공간이 달라짐) 현재 통로는 다 다르지만 변수는 같음
	...
	public static void main(String[] args) {
		...
		try {
			...
			while(true) {
				...
				SocketServer socketServer =new SocketServer(socketUser);// 클라이언트 받아들일
